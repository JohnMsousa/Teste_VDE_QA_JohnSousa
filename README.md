...
O relatório será gerado em: `mochawesome-html/mochawesome.html` com estatísticas, detalhes dos testes, screenshots de falhas e tempo de execução.
...
⚠️ **Requisito:** Configure os GitHub Secrets `TEST_USER` e `TEST_PASSWORD` para execução dos testes no CI/CD.
...
4. Escreva os testes seguindo o padrão Page Objects (veja exemplo na seção [Padrão Page Objects](#-padrão-page-objects))
...
3. **Problemas de login**
    ...
    - Verifique se as credenciais estão configuradas no `LoginPage.js` ou nos GitHub Secrets
